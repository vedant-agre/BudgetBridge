...
import { Navigation, CreditCard, Star, ExternalLink, MapPin } from "lucide-react";
    ...
    <div className="relative pl-8 md:pl-0">
      ...
      >
        {dayData.places.map((place, idx) => {
          const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${place.geoCoordinates?.lat},${place.geoCoordinates?.lng}`;
            ...
            <motion.div variants={itemVariants} key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-3rem)] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-5 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                ...
                <div className="flex justify-between items-start mb-3">
                  <h4 className="font-bold text-lg text-zinc-800 dark:text-zinc-100 flex items-start gap-2">
                    <MapPin className="w-5 h-5 text-emerald-500 shrink-0 mt-0.5" />
                    <a 
                      href={mapsUrl} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors flex flex-wrap items-center gap-1.5"
                  ...
                  {place.rating > 0 && (
                    <div className="flex items-center gap-1 text-yellow-500 text-xs font-bold shrink-0 ml-2">
